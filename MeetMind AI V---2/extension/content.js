...
const extensionStatusJSON_bug = {
  "status": 400,
  "message": "<strong>MeetMind AI encountered a new error</strong> <br /> Please report it <a href='https://github.com/SU1NEG/MeetMind-AI/issues' target='_blank'>here</a>."
}
const reportErrorMessage = "There is a bug in MeetMind AI. Please report it at https://github.com/SU1NEG/MeetMind-AI/issues"
...
function meetingRoutines(uiType) {
  ...
  waitForElement(meetingEndIconData.selector, meetingEndIconData.text).then(() => {
    ...
    if (!isTranscriptDomErrorCaptured && !isChatMessagesDomErrorCaptured) {
      chrome.storage.sync.get(["operationMode"], function (result) {
        if (result.operationMode == "manual") {
          showNotification({ status: 400, message: "<strong>MeetMind AI is not running</strong> <br /> Turn on captions using the CC icon, if needed" })
        }
        else {
          showNotification(extensionStatusJSON)
        }
...
async function checkExtensionStatus() {
  ...
  chrome.storage.local.set({
    extensionStatusJSON: { status: 200, message: "<strong>MeetMind AI is running</strong> <br /> Do not turn off captions" },
  });
