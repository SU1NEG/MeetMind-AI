  ...
  if (!recognition) {
    ...
    recognition.onresult = (event) => {
      ...
      if (finalTranscript !== "") {
        ...
        console.log("Mevcut transcription:", transcription); // Transcription'ı logluyoruz
      }
...
function stopSpeechRecognition() {
  ...
  if (recognition) {
    ...
    console.log("Metin uzunluğu:", trimmedTranscription.length);
    ...
    if (trimmedTranscription) {
      ...
      console.log(
        "Kaydedilecek toplantı verileri:",
...
async function saveMeetingData(meetingData) {
  try {
    console.log("saveMeetingData çağrıldı, veri:", JSON.stringify(meetingData));

    const result = await chrome.storage.local.get(["meetings"]);
    console.log("Mevcut toplantılar:", result.meetings);

    const meetings = result.meetings || [];
    meetings.push(meetingData);

    console.log("Güncellenmiş toplantılar:", meetings);

    await chrome.storage.local.set({ meetings });

    // Doğrulama için verileri tekrar alıyoruz
